...
class pos_user_product(report_sxw.rml_parse):
    ...
    def _get_user(self, object):
        sql = """select name from account_bank_statement as abs,res_users ru
                                    where abs.user_id = ru.id
                                    and abs.id = %d"""%(object.id)
        self.cr.execute(sql)
...
report_sxw.report_sxw('report.pos.user.product.webkit', 'account.bank.statement', 'addons/point_of_sale_webkit/report/pos_users_product.mako', parser=pos_user_product,header='internal')
